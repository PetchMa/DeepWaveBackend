...
def upload_file():
	if request.method == 'POST':
		try:
			...
			f.save(os.path.join(BUCKET_NAME, secure_filename(f.filename)))
...
def denoise(filename):
	try:	
		img = 0
		imgData = os.path.join(BUCKET_NAME, filename)
		gcs_file = cloudstorage.open(imgData)
		print("Check")
		...
		with graph.as_default():
			img = model.predict(x)
			img = img [0, : , :, :]
			cleaned_path = BUCKET_NAME + 'cleaned-' + filename
